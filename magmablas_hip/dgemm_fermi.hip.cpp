...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas/zgemm_fermi.cu, normal z -> d, Mon Feb  5 14:57:29 2024
    ...
    magma_queue_t queue )
{
         ...
         sizeB >= CUBLAS_MAX_1DBUF_SIZE )
    {
        ...
        tex_ref_Amagma_d.filterMode = hipFilterModePoint;
        tex_ref_Amagma_d.addressMode[0] = hipAddressModeClamp;

        tex_ref_Bmagma_d.normalized = false;
        tex_ref_Bmagma_d.filterMode = hipFilterModePoint;
        tex_ref_Bmagma_d.addressMode[0] = hipAddressModeClamp;

        // Bind A and B to texture references
        hipError_t err;
        err = hipBindTexture(&offsetA, tex_ref_Amagma_d, dA, sizeA*sizeof(double));
        if ( err != hipSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", hipGetErrorString(err), err );
            return;
        }
        err = hipBindTexture(&offsetB, tex_ref_Bmagma_d, dB, sizeB*sizeof(double));
        if ( err != hipSuccess ) {
    ...
    if ( TransA == 0 && TransB == 0 ) {
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_nn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_nt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_nc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_tn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_tt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_tc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_cn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_ct, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(dgemm_kernel_fermi_cc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
    ...
    #ifdef TEXTURE_1D
        hipUnbindTexture( tex_ref_Amagma_d );
        hipUnbindTexture( tex_ref_Bmagma_d );
    #endif
}
