...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas/zgemm_fermi.cu, normal z -> s, Mon Feb  5 14:57:29 2024
    ...
    magma_queue_t queue )
{
         ...
         sizeB >= CUBLAS_MAX_1DBUF_SIZE )
    {
        ...
        tex_ref_Amagma_s.filterMode = hipFilterModePoint;
        tex_ref_Amagma_s.addressMode[0] = hipAddressModeClamp;

        tex_ref_Bmagma_s.normalized = false;
        tex_ref_Bmagma_s.filterMode = hipFilterModePoint;
        tex_ref_Bmagma_s.addressMode[0] = hipAddressModeClamp;

        // Bind A and B to texture references
        hipError_t err;
        err = hipBindTexture(&offsetA, tex_ref_Amagma_s, dA, sizeA*sizeof(float));
        if ( err != hipSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", hipGetErrorString(err), err );
            return;
        }
        err = hipBindTexture(&offsetB, tex_ref_Bmagma_s, dB, sizeB*sizeof(float));
        if ( err != hipSuccess ) {
            fprintf( stderr, "cannot bind B to texture: %s (%d)\n", hipGetErrorString(err), err );
            hipUnbindTexture( tex_ref_Amagma_s );
            return;
        }
    #endif
    ...
    if ( TransA == 0 && TransB == 0 ) {
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_nn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_nt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_nc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_tn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_tt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_tc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_cn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_ct, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(sgemm_kernel_fermi_cc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
    ...
    #ifdef TEXTURE_1D
        hipUnbindTexture( tex_ref_Amagma_s );
        hipUnbindTexture( tex_ref_Bmagma_s );
    #endif
}
