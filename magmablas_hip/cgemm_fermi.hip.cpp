...
/*
    -- MAGMA (version 2.0) --
       ...
       @generated from magmablas/zgemm_fermi.cu, normal z -> c, Mon Feb  5 14:57:30 2024
       ...
       gemm_stencil.hip.hpp             defines the GPU kernel (device function).
       gemm_kernel.hip.hpp              defines the GPU kernel (global function).
       
       The batched version uses gemm_kernel_batched.hip.hpp instead of gemm_kernel.hip.hpp.
*/
    ...
    magma_queue_t queue )
{
         ...
         sizeB >= CUBLAS_MAX_1DBUF_SIZE )
    {
        ...
        tex_ref_Amagma_c.filterMode = hipFilterModePoint;
        tex_ref_Amagma_c.addressMode[0] = hipAddressModeClamp;

        tex_ref_Bmagma_c.normalized = false;
        tex_ref_Bmagma_c.filterMode = hipFilterModePoint;
        tex_ref_Bmagma_c.addressMode[0] = hipAddressModeClamp;

        // Bind A and B to texture references
        hipError_t err;
        err = hipBindTexture(&offsetA, tex_ref_Amagma_c, dA, sizeA*sizeof(magmaFloatComplex));
        if ( err != hipSuccess ) {
            fprintf( stderr, "cannot bind A to texture: %s (%d)\n", hipGetErrorString(err), err );
            return;
        }
        err = hipBindTexture(&offsetB, tex_ref_Bmagma_c, dB, sizeB*sizeof(magmaFloatComplex));
        if ( err != hipSuccess ) {
            fprintf( stderr, "cannot bind B to texture: %s (%d)\n", hipGetErrorString(err), err );
            hipUnbindTexture( tex_ref_Amagma_c );
            return;
        }
    #endif
    ...
    if ( TransA == 0 && TransB == 0 ) {
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_nn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_nt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_nc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_tn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_tt, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_tc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_cn, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_ct, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
        ...
        hipLaunchKernelGGL(cgemm_kernel_fermi_cc, dim3(dimGrid), dim3(dimBlock), 0, queue->hip_stream() , 
            m, n, k, dA, ldda, dB, lddb, dC, lddc, alpha, beta,
            (int)offsetA, (int)offsetB );
    }
    ...
    #ifdef TEXTURE_1D
        hipUnbindTexture( tex_ref_Amagma_c );
        hipUnbindTexture( tex_ref_Bmagma_c );
    #endif
}
