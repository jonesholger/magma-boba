    ...
    magma_queue_t queue)
{
    ...
    while (out_flag)
	{
	    //magma_dgemv(MagmaNoTrans, n, n, c_neg_one, dA, ldda, dX, 1, c_zero, d_VV, 1, queue); // d_VV = -A*x
	    ...
	    while (in_flag) {
            ...
            //{
                // magma_ccopy( n, &d_VV[i*n2], 1, &d_w[i*n2], 1, queue);
                // magma_cgetrs_gpu( MagmaNoTrans, n, nrhs, dLU, lddlu, ipiv, &d_w[i*n2], n2, &info );
                //magma_dsgetrs_gpu( MagmaNoTrans, n, nrhs, dLU_sprec, lddlusp, dipiv, &d_VV[i*n2], n2, &d_w[i*n2], n2, dSX, &info );
		    ...
		    //magma_dgemv( MagmaNoTrans, n, n, c_one, dA, ldda, &d_w[i*n2], 1, c_zero, &d_VV[(i+1)*n2], 1, queue);
	    	...
	    	if (fabs(gam-MAGMA_D_ZERO) <= EPSILON)
		        gam = EPSMAC;
		    ...
		    */
		    ...
		    if (i+1 >=restrt || ro <= eps1 || iters >= maxiter) {
		        //MAGMA_PRINTF("--> i+1 = %d, restrt = %d, ro = %e, eps1 = %e, iters = %d, maxiter = %d\n", i+1, restrt, ro, eps1, iters, maxiter);
		        in_flag = false;
		    }
	    } /* end of inner loop */
    ...
    *niters = tot_inner_iter > 0 ? tot_inner_iter : iters; 
    if(!is_inner)
    {    
        //MAGMA_PRINTF("\n\ninfo===> outer-iter %lld  inner-iter is %d \n\n", (long long) iters, tot_inner_iter); 
    }
    /*------------------------------------------------------*
    ...
    //magma_cprint_gpu(n, nrhs, dX, lddx, queue);
