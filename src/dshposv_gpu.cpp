...
/***************************************************************************//**
    ...
    @param
    dworkd  (workspace) DOUBLE PRECISION array on the GPU, dimension (N*NRHS) + N
    ...
    float cn, float theta, magma_int_t *info)
{
    ...
    *info = 0;
    ...
    // http://eprints.maths.manchester.ac.uk/2736/
    const float cn_f  = (float)cn;
    const float eps_h = (1.0 / 2048.0); //0.00097656; //lapackf77_slamch("Epsilon"); // eps for h
    ...
    //printf("dA = ");magma_dprint_gpu(n, n, dA, ldda, queue);
    if(preprocess > 0) {
    ...
    //printf("info before cholesky = %lld \n", (long long)(*info) );
    //printf("dSA = ");magma_sprint_gpu(n, n, dSA, lddsa, queue);
    if(0) {
        magma_spotrf_gpu( uplo, n, dSA, lddsa, info );
    }
    else {
        magma_int_t nb = 512, recnb = 128;
        magma_shpotrf_LL_expert_gpu(uplo, n, dSA, lddsa, nb, recnb, info, mode );
    }
